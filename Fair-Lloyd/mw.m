...
for t=1:T
    ...
    %record the progress
    P_average = (1/t).*P;
    record = [record; t w_1 w_2 z_1 z_2 (1/m_A)*(alpha - sum(sum( covA .* P_average )))...
        (1/m_B)*(beta - sum(sum( covB .* P_average )))];
end
...
if ~isequal(size(A), [m_A,n]) || ~isequal(size(B), [m_B,n]) %wrong size
